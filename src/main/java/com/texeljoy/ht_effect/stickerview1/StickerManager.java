...
public class StickerManager {
     ...
     * @return
     */
    public boolean getDelButton(float x, float y) {
    ...
    /**
     * 根据触摸是否触摸到旋转按钮，返回对应旋转按钮的贴纸
     *
     * @param x
     * @param y
     * @return
     */
    public boolean getRotateButton(float x, float y) {
        ...
        for (int i = mStickerList.size() - 1; i >= 0; i--) {
            ...
            sticker.getMatrix().invert(matrix);
            matrix.mapPoints(dstPoints, srcPoints);
                if (sticker.getRotateBitmapBound().contains(dstPoints[0], dstPoints[1])) {
                return true;
            }
        }
        return false;
