...
public class StickerLayout extends View implements View.OnTouchListener {
    ...
    protected void onDraw(Canvas canvas) {
        ...
        if (focusSticker != null) {
            ...
            float[] points = focusSticker.getmDstPoints();
            int width = (int)(points[2] - points[0]);
            int height = (int)(points[5] - points[3]);
//            HTEffect.shareInstance().setWatermarkParam((float) (points[0] / mWidth),(float) (points[1] / mHeight),width,height,0);
        }
    }
    ...
    public boolean onTouch(View v, MotionEvent event) {
        switch (event.getAction() & MotionEvent.ACTION_MASK) {
            ...
            case MotionEvent.ACTION_POINTER_DOWN:
                //判断是否按到删除按钮
                isDelete = StickerManager.getInstance().getDelButton(event.getX(), event.getY());
                // if (mStick != null) {
                ...
                //判断是否按到旋转按钮
                isRotate = StickerManager.getInstance().getRotateButton(event.getX(),event.getY());
                if (isRotate) {
